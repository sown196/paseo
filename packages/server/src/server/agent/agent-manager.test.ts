...
test.each([false, true])(
  "provider policy only narrows the allowlist with native tools=%s",
  async (supportsNativePaseoTools) => {
    const workdir = mkdtempSync(join(tmpdir(), "agent-manager-provider-policy-intersection-"));
    class PolicyCaptureClient extends McpCapableTestAgentClient {
      override readonly capabilities = {
        ...TEST_CAPABILITIES,
        supportsMcpServers: true,
    ...
    const lead = new PolicyCaptureClient("codex-lead");
    const peer = new PolicyCaptureClient("codex-peer");
    const policies = new Map<AgentProvider, { enabled?: boolean; disabledTools?: string[] }>([
      ["codex-lead", { disabledTools: ["create_agent"] }],
      ["codex-peer", { enabled: true }],
    ]);
    ...
    const manager = new AgentManager({
      clients: { "codex-lead": lead, "codex-peer": peer },
      logger,
      mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
      paseoToolProviderIds: ["codex-lead"],
      resolvePaseoToolPolicy: (provider) => policies.get(provider),
      paseoToolCatalogFactory: (context) => {
        policyInputs.push(context.paseoToolPolicy);
        return paseoTools;
      },
    });

    try {
      const allowed = await manager.createAgent(
        { provider: "codex-lead", cwd: workdir },
        undefined,
        { workspaceId: undefined },
      );
      const denied = await manager.createAgent(
        { provider: "codex-peer", cwd: workdir },
        undefined,
        { workspaceId: undefined },
      );
      expect(manager.getPaseoToolPolicy(allowed.id)).toEqual({
        disabledTools: ["create_agent"],
      });
      expect(manager.getPaseoToolPolicy(denied.id)).toEqual({ enabled: false });
      expect(peer.createdConfigs[0]?.mcpServers?.paseo).toBeUndefined();
      expect(peer.launchContexts[0]?.paseoTools).toBeUndefined();
      expect(policyInputs).toEqual(
        supportsNativePaseoTools ? [{ disabledTools: ["create_agent"] }] : [],
      );

      policies.set("codex-lead", { enabled: false });
      const disabled = await manager.createAgent(
        { provider: "codex-lead", cwd: workdir },
        undefined,
        { workspaceId: undefined },
      );
      expect(manager.getPaseoToolPolicy(disabled.id)).toEqual({ enabled: false });
      expect(lead.createdConfigs[1]?.mcpServers?.paseo).toBeUndefined();
      expect(lead.launchContexts[1]?.paseoTools).toBeUndefined();
    } finally {
      rmSync(workdir, { recursive: true, force: true });
